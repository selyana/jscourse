...
let scrollCheck = quarySelector('.scrollCheck');
...
function init() {
    ...
    if (window.innerHeight === document.body.offsetHeight) { // можно while ? 
        // и почему в условии не "меньше или равно", ведь бади изначально меньше вьюпорта? 
        insertPosts();
        init();
    }
}
...
function insertPosts() {
    let postsMarkup = '';
    for (let i = 0; i < 3; i++) { //почему 3? потому что столько помещается на страницу? 
 ...
 */
function getPostMarkup(counter) { //сюда передаются 0, 1, 2? (счетчик i)

    let num = new Date().getTime() + counter; // зачем прибавлять счетчик? 
